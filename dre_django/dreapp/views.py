...
def browse_day( request, year, month, day ):
    ...
    for obj in context['page'].object_list:
        if obj.no_index() and not request.user.is_authenticated():
            context['forgetme'] = True
            break
...
def document_display( request, docid ):
    ...
    # TODO: retrieve the html text between these dates and remove the
    # warning altogether
    context['pdf_warning'] = (document.date >= datetime.date(2016,3,24) and
                              document.date < datetime.date(2016, 12, 19))
